...
const AIRankerChat = () => {
  ...
  const confirmDelete = async () => {
    ...
    const chatIdToDelete = chatToDelete.id || chatToDelete._id;
    ...
    try {
      ...
      setChatHistory((prev) => prev.filter((c) => {
        ...
        return cId !== chatIdToDelete;
      }));

      // ✅ If current chat was deleted, navigate back
  ...
  const formatMessage = (content) => {
    ...
    return lines.map((line, i) => {
        ...
        return (
          <p key={i} className="text-sm leading-relaxed">
            {parts.map((part, idx) => {
              const partStr = part || "";
              if (idx % 2 === 1) {
                return <strong key={idx} className="text-gray-800">{partStr}</strong>;
              }
              return <span key={idx} className="text-gray-700">{partStr}</span>;
            })}
          </p>
        );
      }
      ...
      return <p key={i} className="text-sm leading-relaxed text-gray-700">{lineStr}</p>;
    });
  };
