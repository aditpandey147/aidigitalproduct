...
const AIRankerChat = () => {
  ...
  const loadChat = async () => {
    ...
    try {
      ...
      if (response.data?.success) {
        ...
        console.log('✅ Ranker chat loaded:', response.data.chat.agentName);
  ...
  const loadChatHistory = async (agentSlug) => {
    try {
      ...
      if (response.data?.success) {
        ...
        console.log(`✅ Loaded ${filtered.length} chat history for agent:`, agentSlug);
      }
    } catch (error) {
  ...
  const sendMessage = async () => {
    ...
    try {
      const response = await api.post(`/ai-ranker/chat/${chatId}`, { message });
  ...
  const formatMessage = (content) => {
    ...
    return content
      ...
      .map((line, i) => {
        if (line.startsWith('## ')) {
          return <h3 key={i} className="text-base font-bold mt-4 mb-1 text-gray-800">{line.slice(3)}</h3>;
        }
        if (line.startsWith('### ')) {
          return <h4 key={i} className="text-sm font-semibold mt-3 mb-1 text-gray-700">{line.slice(4)}</h4>;
        }
        if (line.includes('**') && !line.includes('```')) {
          ...
          return (
            <p key={i} className="text-sm leading-relaxed">
              {parts.map((part, idx) => 
                idx % 2 === 1 ? <strong key={idx} className="text-gray-800">{part}</strong> : <span key={idx} className="text-gray-700">{part}</span>
              )}
            </p>
          );
        }
        ...
        if (/^\d+\./.test(line)) {
          return (
            <div key={i} className="flex items-start gap-2 ml-2">
              <span className="text-emerald-500 text-sm font-medium mt-0.5">{line.match(/^\d+/)[0]}.</span>
              <span className="text-sm text-gray-700">{line.replace(/^\d+\./, '')}</span>
            </div>
          );
        }
        if (line.startsWith('```') && line.endsWith('```')) {
          return (
            <div key={i} className="bg-gray-900 text-green-400 p-4 rounded-xl my-2 text-xs font-mono overflow-x-auto">
              {line.slice(3, -3)}
            </div>
          );
        }
        if (line.includes('`') && !line.includes('```')) {
          ...
          return (
            <p key={i} className="text-sm leading-relaxed">
              {parts.map((part, idx) => 
                idx % 2 === 1 ? (
                  <code key={idx} className="bg-gray-100 px-1.5 py-0.5 rounded text-xs font-mono text-emerald-600">
                    {part}
                  </code>
                ) : (
                  <span key={idx} className="text-gray-700">{part}</span>
                )
              )}
            </p>
          );
                  ...
                  <div className="flex items-end gap-3 animate-slideIn">
                    ...
                    <div className="bg-white border border-gray-100 shadow-md rounded-2xl rounded-tl-none px-5 py-3">
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-xs text-gray-400">Analyzing SEO...</span>
                      </div>
                    </div>
                  </div>
                )}
            ...
            <div className="flex flex-col h-full">
              ...
              <div className="flex-shrink-0 px-4 py-4 border-b border-gray-100 bg-gradient-to-r from-emerald-50/50 to-teal-50/50">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <h4 className="text-sm font-semibold text-gray-800">SEO Chat History</h4>
              ...
              <div className="flex-1 overflow-y-auto px-2 py-3">
                {chatHistory.length === 0 ? (
                  <div className="flex flex-col items-center justify-center h-full text-center px-4">
                    ...
                    <p className="text-sm font-medium text-gray-500">No previous chats</p>
                    <p className="text-xs text-gray-400 mt-1">Start a new SEO conversation</p>
                  </div>
                ) : (
                  chatHistory.map((chatItem) => (
                    <button
                      ...
                      className="w-full text-left px-3 py-2.5 rounded-xl transition-all duration-200 group flex items-start gap-3 mb-1 hover:bg-gray-50 border-2 border-transparent hover:border-emerald-200"
                    >
                      <div className="w-8 h-8 rounded-lg bg-gray-100 flex items-center justify-center flex-shrink-0 mt-0.5">
                        <i className={`fas ${agent.icon} text-emerald-500 text-xs`}></i>
                      </div>
                      
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center justify-between">
                          <span className="text-sm font-medium truncate text-gray-800 group-hover:text-emerald-600 transition-colors">
